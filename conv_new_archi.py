...
class Net(nn.Module):
    def __init__(self, n_layers, threshold, beta, n_channels, init_func):
        ...
        # First Layer: Convolutional layer + pooling + LIF neurons
        ...
        self.pooling_layers.append(nn.MaxPool2d(kernel_size=2, stride=2))  # Adding max pooling layer
        

        # Hidden Layers: Convolutional layers + pooling + LIF neurons
        ...
        # Final Layer: Flatten + Linear + LIF output neurons
        self.lif_layers.append(snn.Leaky(beta=beta, threshold=threshold, reset_mechanism='subtract'))
        self.layers.append(nn.Linear(num_channels * 4 * 4, num_outputs))  # Adjusted for the smaller feature map size after pooling
        self.lif_layers.append(snn.Leaky(beta=beta, threshold=threshold, reset_mechanism='subtract'))
        ...
        for step in range(time_steps):
            ...
            for i in range(len(self.lif_layers)):
                if isinstance(self.layers[i], nn.Conv2d):
                    cur = self.layers[i](cur)  # Apply convolutional layer
                    cur = self.pooling_layers[i](cur)  # Apply pooling layer
                    spk, mems[i] = self.lif_layers[i](cur, mems[i])  # Apply LIF neuron
                elif isinstance(self.layers[i], nn.Linear):
                    cur = cur.view(cur.size(0), -1)  # Flatten the spike tensor before passing to linear layer
                    spk, mems[i] = self.lif_layers[i](cur, mems[i])  # Apply LIF neuron
...
def training(net, time_steps, train_loader, test_loader):
    ...
    for epoch in range(num_epochs):
        ...
        for data, targets in train_loader:
            ...
            #spike_counts_batch_train.append(spike_counts_list)
        ...
        with torch.no_grad():
            ...
            for test_data, test_targets in test_loader:
                ...
                #spike_counts_batch_test.append(spike_counts_list_test)
        ...
        #spike_counts_train_epoch = [np.mean(np.array(spike_counts), axis=0) for spike_counts in spike_counts_batch_train]
        #spike_counts_train.append(spike_counts_train_epoch)

        # Compute and store mean SPIKE COUNTS over batches for test
        #spike_counts_test_epoch = [np.mean(np.array(spike_counts), axis=0) for spike_counts in spike_counts_batch_test]
