...
class Net(nn.Module):
    ...
    def forward(self, input_tensor, time_steps):
        ...
        #mean_var_mems_list = [torch.tensor(var_list).mean() for var_list in var_mems_list]
        #mean_spike_counts_list = [torch.tensor(spike_list).mean(dim=0) for spike_list in spike_counts_list]
        #mean_var_mems_list = [np.array(var_list).mean() for var_list in var_mems_list]
        #print(np.array(mean_var_mems_list[0].shape))
        #mean_spike_counts_list = [np.mean(np.array(spike_list), axis=0) for spike_list in spike_counts_list]
        #print(mean_spike_counts_list)
        # Stack spike_out into a tensor
        if spike_out:
            spike_out = torch.stack(spike_out, dim=0)
        else:
            spike_out = torch.tensor([], device=input_tensor.device)
        #print(len(spike_counts_list))
        return spike_out
...
def main(args):
    ...
    stat_filename = f'stat_{dataset_name}_{init_func_name}_layers_{n_layers}_thresh_{threshold}_beta_{beta}_neurons_{n_neurons}_timesteps_{time_steps}.npy'
    #var_y_filename = f'var_spk_{dataset_name}_{init_func_name}_layers_{n_layers}_thresh_{threshold}_beta_{beta}_neurons_{n_neurons}_timesteps_{time_steps}.npy'
