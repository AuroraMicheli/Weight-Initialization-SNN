...
def load_CIFAR10():
    ...
    batch_size = 64
    data_path = 'path/to/data/cifar10'
    subset_size_train = 25000 # Define the size of the subset you want
    ...
    cifar10_train = data_subset(cifar10_train, subset_size_train)
    cifar10_test = data_subset(cifar10_test, subset_size_test)
...
def kaiming_init_uniform_self(layer):
    if isinstance(layer, nn.Conv2d):
        n_input = layer.weight.size(1) * layer.weight.size(2) * layer.weight.size(3)
        var_w = 2/n_input
...
def kaiming_init_normal(layer):
    if isinstance(layer, nn.Conv2d):
        ...
        weight_var = torch.var(layer.weight).item()
        print(f"Variance of weights for layer {layer}: {weight_var:.6f}")
