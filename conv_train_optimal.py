...
class Net(nn.Module):
    def __init__(self, n_layers, threshold, n_channels, init_func):
        super().__init__()
        ...
        self.layers.append(nn.Conv2d(3, num_channels, kernel_size=3, stride=1, padding=1)) 
        ...
        self.lif_layers.append(snn.Leaky(beta=beta, threshold=threshold, reset_mechanism='subtract'))
        self.layers.append(nn.Linear(num_channels * 32 * 32, num_outputs)) 
        self.lif_layers.append(snn.Leaky(beta=beta, threshold=threshold, reset_mechanism='subtract'))
    ...
    def forward(self, input_tensor, time_steps):
        ...
        spike_vectors = []  # To store spike vectors for the last layer
        mems = [layer.init_leaky() for layer in self.lif_layers]

        var_y_list = [[] for _ in range(len(self.layers))]  # Initialize lists of lists for var y values
        cur_list = [[] for _ in range(len(self.layers))]  # Initialize lists of lists for cur values (=y) 

        for step in range(time_steps):
            for i in range(len(self.lif_layers)):
                if i == (len(self.lif_layers) - 1): #last lif layer: only append spk out
...
def training(net, time_steps):
    ...
    for epoch in range(num_epochs):
        ...
        var_y_batch_test = [] 
        #cur_batch_train = []  # Collect variance data for each batch in this epoch
        #cur_batch_test = [] 
        ...
        for data, targets in train_batch:
            ...
            #cur_batch_train.append(cur_list)
        ...
        with torch.no_grad():
            ...
            for test_data, test_targets in test_loader:
                ...
                #cur_batch_test.append(cur_list_test)
       ...
       # prin('done first epoch')
        ...
        #cur_train.append(np.mean(np.array(cur_batch_train), axis=0))  # Append variance data for this epoch
        #cur_test.append(np.mean(np.array(cur_batch_test), axis=0))
...
def main(args):
    ...
    net=Net(n_layers, threshold, n_channels, init_func).to(device)
    loss_hist, test_loss_hist, acc_hist, test_acc_hist, var_y_train, var_y_test = training(net, time_steps)
    
    stat_filename = f'stat_cifar10_{init_func_name}_layers_{n_layers}_thresh_{threshold}_channels_{n_channels}_timesteps_{time_steps}.npy'
    var_y_filename = f'var_y_cifar10_{init_func_name}_layers_{n_layers}_thresh_{threshold}_channels_{n_channels}_timesteps_{time_steps}.npy'
    ...
    np.save(os.path.join('/tudelft.net/staff-bulk/ewi/insy/VisionLab/amicheli/mlpSNN/weights_variance/results/cifar10_optimal', stat_filename), np.vstack((loss_hist, test_loss_hist, acc_hist, test_acc_hist))) #return a file with x=epochs, y_1line=loss, y_2line=test_loss, y_3line=acc, y_4line=test acc
    np.save(os.path.join('/tudelft.net/staff-bulk/ewi/insy/VisionLab/amicheli/mlpSNN/weights_variance/results/cifar10_optimal', var_y_filename), np.array([var_y_train, var_y_test]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_channels', type=int, default=100, help='number of channels in conv layers')
