...
class Net(nn.Module):
    def __init__(self, n_layers, threshold, beta, n_neurons, init_func, surrogate):
        super().__init__()
        ...
        self.lif_layers.append(snn.Leaky(beta=beta, threshold=threshold, reset_mechanism='subtract', spike_grad = spike_grad))
...
def training(net, time_steps, train_loader, test_loader):
    ...
    return loss_epoch_hist, test_loss_epoch_hist, acc_epoch_hist, test_acc_epoch_hist, var_mems_train, var_mems_test, spike_counts_train, spike_counts_test
...
def main(args):
    ...
    if surrogate_name not in surrogates:
        raise ValueError(f"Invalid surrogate function: {surrogate_name}")

    
    surrogate_func = surrogates[surrogate_name]
    ...
    net=Net(n_layers, threshold, beta, n_neurons, init_func, surrogate_func).to(device)
    loss_hist, test_loss_hist, acc_hist, test_acc_hist, var_mems_train, var_mems_test, spike_counts_train, spike_counts_test = training(net, time_steps, train_loader, test_loader)
    
    stat_filename = f'surr_{surrogate_name}_stat_{dataset_name}_{init_func_name}_layers_{n_layers}_thresh_{threshold}_beta_{beta}_neurons_{n_neurons}_timesteps_{time_steps}.npy'
    spk_filename = f'surr_{surrogate_name}_spk_{dataset_name}_{init_func_name}_layers_{n_layers}_thresh_{threshold}_beta_{beta}_neurons_{n_neurons}_timesteps_{time_steps}.npy'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--surrogate', type=str, default='atan', help='surrogate gradient')
