...
def training(net, time_steps, weight_decay, lr, train_loader, test_loader):
    dtype = torch.float
    ...
    optimizer = torch.optim.Adam(net.parameters(), lr=lr, weight_decay = weight_decay , betas=(0.9, 0.999))
...
def main(args):
    ...
    loss_hist, test_loss_hist, acc_hist, test_acc_hist, var_mems_train, var_mems_test, spike_counts_train, spike_counts_test = training(net, time_steps, weight_decay, lr, train_loader, test_loader)
    
    stat_filename = f'stat_{dataset_name}_{init_func_name}_layers_{n_layers}_wd_{weight_decay}_lr_{lr}.npy'
    spk_filename = f'spk_{dataset_name}_{init_func_name}_layers_{n_layers}_wd_{weight_decay}_lr_{lr}.npy'
